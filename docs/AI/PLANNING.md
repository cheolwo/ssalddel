...
| `PLAN-SYSTEM-MYEONMOK-OBSERVER` | [현재 동네 전체 맵·코드 선행 정리 r1](Planning/시스템/PLAN-SYSTEM-MYEONMOK-OBSERVER/neighborhood-maps.r1.md) | `Approved / DioramaDirectionRecorded / FiveElementObjectCatalogImplemented / SceneApplicationDeferred / VisualGapsOpen` | [아이소메트릭 3D 디오라마 방향](Planning/시스템/PLAN-SYSTEM-MYEONMOK-OBSERVER/public-data-isometric-diorama-direction.r1.md)과 [오행 업무 객체 대장 r1](Planning/시스템/PLAN-SYSTEM-MYEONMOK-OBSERVER/myeonmok-five-element-game-object-catalog.r1.md)을 결속. Graph Map·배치 Map 뒤 비식별 읽기 전용 자료만 Unity에 연결하며 Scene 적용은 보류. 기존 합성 동네는 별도 시나리오층으로 보존. [오프라인 생활 구성 부록 r1](Planning/시스템/PLAN-SYSTEM-MYEONMOK-OBSERVER/offline-life-bootstrap.r1.md)은 Mongo/JSON·Core·Unity EditMode 검증, 실제 Play/Game View 미검증 |
| `PLAN-SYSTEM-NEIGHBORHOOD-SPATIAL-PACKAGES` | [법정동별 공간 패키지와 공통 오행 업무 객체 r1](Planning/시스템/PLAN-SYSTEM-NEIGHBORHOOD-SPATIAL-PACKAGES/README.md) | `Approved / FoundationImplemented / MyeonmokPlacementCompatibilityPreserved / JunghwaInventoryReady / UnityApplicationDeferred` | 공통 오행 객체 대장을 한 번만 두고 법정동을 저장 식별자, 500m 타일을 후속 표시·로드 단위로 분리한다. 면목동 Graph/배치 호환을 보존하고 중화동은 개인정보를 줄인 공공자료 관측 재고까지만 등록한다. |
